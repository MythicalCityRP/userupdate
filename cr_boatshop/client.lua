...
cDetails = {
	...
	worldTime = {0, 0},
	position = Vector3(741.48449707031, -1478.3039550781, 4.3),
	marker = false,
	greenBox = false,
	controllerPed = false,
	vehiclePreviewPosition = Vector3(726.06549072266, -1517.37890625, -0.68046522140503),
	mode = "show",
	isShowing = false,
	selectedBrand = 1,
...
function renderBoatshop()
	...
	dxDrawText('Tienda de Botes', 115, 100/2, 115, 100/2 + 4, tocolor(242, 242, 242, alpha), 1, font, "left", "center")
	...
	dxDrawText('Velocidad máxima', x + 30, y + 85, x + 30, y + 85, tocolor(242, 242, 242, alpha), 1, font2, "left", "top")
	...
	dxDrawText('Adhesión', x + 30, y + 125, x + 30, y + 125, tocolor(242, 242, 242, alpha), 1, font2, "left", "top")
	...
	dxDrawText('Frenos', x + 30, y + 170, x + 30, y + 170, tocolor(242, 242, 242, alpha), 1, font2, "left", "top")
	...
	local limit = tostring(carList[cDetails.selectedBrand].vehicles[cDetails.selectedVehicle].limit).. " slots"
	if tonumber(carList[cDetails.selectedBrand].vehicles[cDetails.selectedVehicle].limit) <= 0 then 
		limit = "Ilimitado"
	end 

	local text = 'Consumo: '..(cDetails.vehDetails.values['vehicleConsumption'].new).." L/100KM\nTipo de combustible: "..(cDetails.vehDetails.values['vehicleFuelType'].new).."\nTracción: "..(cDetails.vehDetails.values['vehicleDrive'].new).."\nVelocidad máxima: "..(cDetails.vehDetails.values['maximumVelocity'].new).." KM/h\nLímite: "..limit
	...
	if exports['cr_core']:isInSlot(x + w - 150, y - 5 - 30, 150, 30) then 
		...
		exports['cr_dx']:dxDrawImageWithText(":cr_boatshop/files/imgs/testdriveIcon.png", 'Probar', x + w - 150 + 150/2, y - 5 - 30, x + w - 150 + 150/2, y - 5 - 30 + 30 + 4, tocolor(51, 51, 51, alpha), tocolor(51, 51, 51, alpha), 20, 20, 1, font5, 5, -2)
	else 
		dxDrawRectangle(x + w - 150, y - 5 - 30, 150, 30, tocolor(51, 51, 51, alpha * 0.6))

		exports['cr_dx']:dxDrawImageWithText(":cr_boatshop/files/imgs/testdriveIcon.png", 'Probar', x + w - 150 + 150/2, y - 5 - 30, x + w - 150 + 150/2, y - 5 - 30 + 30 + 4, tocolor(242, 242, 242, alpha * 0.6), tocolor(242, 242, 242, alpha * 0.6), 20, 20, 1, font5, 5, -2)
	end 
	...
	if exports['cr_core']:isInSlot(x, startY, w, 30) then 
		...
		dxDrawText('Comprar', x, startY, x + w, startY + 30 + 4, tocolor(242, 242, 242, alpha), 1, font5, "center", "center")
	else 
		dxDrawRectangle(x, startY, w, 30, tocolor(97, 177, 90, alpha * 0.7))
		dxDrawText('Comprar', x, startY, x + w, startY + 30 + 4, tocolor(242, 242, 242, alpha * 0.6), 1, font5, "center", "center")
	end 
...
function setViewerCar(model)
	...
	cDetails.vehDetails.values["vehicleDrive"].new = getHandlingProperty(cDetails.vehicle, "driveType") == "awd" and "4 ruedas" or (getHandlingProperty(cDetails.vehicle, "driveType") == "rwd" and "Rueda trasera" or "Rueda delantera")
	cDetails.vehDetails.values["vehicleFuelType"].new = getHandlingProperty(cDetails.vehicle, "engineType") == "petrol" and "Gasolina" or (getHandlingProperty(cDetails.vehicle, "engineType") == "diesel" and "Dízel" or "Elektromos")
end
	...
	elseif(b == "mouse1") then
		if(state) then
			...
			elseif testDriveHover then 
				...
				local a = 1
				if now <= lastClickTick + a * 1000 then
					exports['cr_infobox']:addBox("warning", "¡Esto no "..a.." se puede a cada segundo!")
					return
				end
					...
					if tonumber(exports['cr_vehicle']:getVehicleCounts()[data.model] or 0) + 1 <= data.limit or data.limit <= 0 then
						if getPlayerVehicles() + 1 <= tonumber(localPlayer:getData("char >> vehicleLimit") or 0) then 
							if tuningPrice[1] == 0 or exports['cr_core']:takeMoney(localPlayer, tuningPrice[1]) then 
								...
								local a = 1
								if now <= lastClickTick + a * 1000 then
									exports['cr_infobox']:addBox("warning", "No "..a.." puedes hacer esto tan rapido")
									return
								end
								...
								exports['cr_infobox']:addBox("success", "Has comprado con éxito el barco por $"..data.price.."")
							else 
								exports['cr_infobox']:addBox("error", "¡No tienes suficiente dinero para comprar este auto!")
							end 
						else
							exports['cr_infobox']:addBox("error", "¡No tienes suficientes slots para comprar este auto!")
						end 
					else 
						exports['cr_infobox']:addBox("error", "¡Este auto está lleno de límites!")
					end
				elseif type == 2 then 
					if tonumber(exports['cr_vehicle']:getVehicleCounts()[data.model] or 0) + 1 <= data.limit or data.limit <= 0 then
						if getPlayerVehicles() + 1 <= tonumber(localPlayer:getData("char >> vehicleLimit") or 0) then 
							if tuningPrice[2] == 0 or exports['cr_core']:takeMoney(localPlayer, tuningPrice[2], true) then 
								...
								local a = 1
								if now <= lastClickTick + a * 1000 then
									exports['cr_infobox']:addBox("warning", "No puedes hacer  "..a.." tan rapido")
									return
								end
								...
								exports['cr_infobox']:addBox("error", "¡No tienes suficiente dinero en tu tarjeta de debito para comprar este auto!")
							end 
						else
							exports['cr_infobox']:addBox("error", "¡No tienes suficientes slots para comprar!")
						end 
					else 
						exports['cr_infobox']:addBox("error", "¡Este auto está lleno de límites!")
					end
				elseif type == 3 then 
					if getPlayerVehicles() + 1 <= tonumber(localPlayer:getData("char >> vehicleLimit") or 0) then 
						if tuningPrice[3] == 0 or localPlayer:getData("char >> premiumPoints") >= tuningPrice[3] then 
							...
							local a = 1
							if now <= lastClickTick + a * 1000 then
								exports['cr_infobox']:addBox("warning", "No puedes hacer esto "..a.." tan rapido")
								return
							end
							...
							toggleBoatshop()
							exports['cr_vehicle']:makeVehicle(localPlayer, data.model, 0, localPlayer:getData("acc >> id"), toJSON(generatePosition()), toJSON({getVehicleColor(cDetails.vehicle, true)}))
							exports['cr_infobox']:addBox("success", "Has comprado con éxito el barco por "..data.pp.." PP")
						else
							exports['cr_infobox']:addBox("error", "¡No tienes suficiente PP para comprar este barco!")
						end 
					else
						exports['cr_infobox']:addBox("error", "¡No tienes suficiente slots para comprar este barco!")
					end 
				end 
			end 
		})
