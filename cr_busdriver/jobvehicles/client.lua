...
function createJobMarkers()
    ...
    local pickupBlip = exports.cr_radar:createStayBlip("Vehiculo trabajo", Blip(jobData.busPickupPoint, 0, 2, 255, 0, 0, 255, 0, 0), 0, "job_vehicle", 24, 24, 255, 255, 255)
...
function deleteJobMarkers()
    for k, v in pairs(createdMarkers) do 
        ...
        if v == "pickupMarker" then 
            exports.cr_radar:destroyStayBlip("Vehiculo trabajo")
...
function setupRoute()
    ...
    local routeBlip = exports.cr_radar:createStayBlip("Siguiente parada", Blip(routePoint, 0, 2, 255, 0, 0, 255, 0, 0), 0, "target", 24, 24, r, g, b)
...
function handleBusStop()
    ...
    outputChatBox(syntax .. "Parada actual: " .. hexColor .. zoneName, 255, 255, 255, true)
    ...
    if jobData.busRoutes[currentRoute + 1] then 
        ...
        outputChatBox(syntax .. "La siguiente parada: " .. hexColor .. zoneName, 255, 255, 255, true)
    end

    currentPedsAtThisStop = #createdPeds

    exports.cr_dx:startProgressBar("busDriver >> progressBar", {
        {"Espere a que los pasajeros despeguen", jobData.pedWaitTime * currentPedsAtThisStop}
    })
...
function payOut()
    ...
    outputChatBox(syntax .. "Has llgado a la parada, hay un total de " .. hexColor .. currentPedsAtThisStop .. " pasajeros " .. white .. " has ganado un total de" .. hexColor .. " " .. payment .. white .. "$.", 255, 255, 255, true)
    ...
    if not jobData.busRoutes[currentRoute + 1] then 
        ...
        outputChatBox(syntax .. "El viaje actual ha terminado, vaya a la primera parada para comenzar un nuevo recorrido.", 255, 255, 255, true)
    end
                ...
                if getDistanceBetweenPoints3D(localPlayer.position, source.position) <= 2.5 then 
                    if localPlayer:getData("char >> jobVehicle") then 
                        exports.cr_jobvehicle:openRequestPanel({
                            type = "destroy",
                            titleText = "Devolver vehiculo Job",
                            buttonText = "Devolver"
                        })
                    else
                    ...
                    if isJobVehicle(modelId) then 
                        if not exports.cr_dx:isProgressBarActive("busDriver >> progressBar") then 
                            if getElementSpeed(vehicle) <= 10 and localPlayer.vehicleSeat == 0 and not gameMinimized then 
                                ...
                                outputChatBox(syntax .. "Fuiste demasiado rápido en la parada.", 255, 0, 0, true)
                            end
                        end
                    end
                end
