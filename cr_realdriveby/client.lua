    ...
    if not doingDriveBY then
        ...
        if windowSeatName and getVehicleComponentVisible(getPedOccupiedVehicle(localPlayer, windowSeatName)) then
            ...
            if not getElementData(veh, "veh >> window"..num.."State") then
                ...
                outputChatBox(syntax .. "¡Solo puedes apoyarte en la ventana abierta!",255,255,255,true)
                return
            end
        end
        ...
        if equipedWeapon ~= 0 and enabledTable[equipedWeapon] then
            ...
            if ammo >= 1 then
                ...
                if ammo <= 0 then
                    local syntax = exports['cr_core']:getServerSyntax(false, "error")
                    outputChatBox(syntax .. "¡Solo tú puedes disparar con una pistola!",255,255,255,true)
                    return
                end
                
                if getElementData(localPlayer, "taser>obj") then
                    local syntax = exports['cr_core']:getServerSyntax(false, "error")
                    outputChatBox(syntax .. "No puedes agacharte con shock",255,255,255,true)
                    return
                end
		...
		exports['cr_chat']:createMessage(localPlayer, "inclinarse por la ventana", 1)
	else
        doingDriveBY = false
		setPedDoingGangDriveby ( localPlayer, false )
		setElementData(localPlayer, "pulling", false)
		exports['cr_chat']:createMessage(localPlayer, "inclinarse hacia el coche", 1)
