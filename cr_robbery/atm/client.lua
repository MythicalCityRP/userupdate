...
function createGrinder()
    ...
    if newState then 
        exports.cr_chat:createMessage(localPlayer, "saca una flexión", 1)
    else
        exports.cr_chat:createMessage(localPlayer, "Alce una flexión", 1)
            ...
            if localPlayer:getData("loggedIn") then 
                if localPlayer:getData("grinderInHand") then 
                    ...
                    if not hasPermissionToRob("atmRob") then 
                        exports.cr_infobox:addBox("warning", "No tienes derecho a hacerlo.")
                        return
                    end
                    ...
                    if isElement(atmObject) then 
                        ...
                        if activeLaw >= 3 then 
                            ...
                            exports.cr_infobox:addBox("error", "Al menos 3 policias conectados se requieren comenzar el robo.")
                        end
                    end
...
function startGrinding(element)
    ...
    if source == localPlayer then 
        if not element:getData("atm >> alreadyNotified") then 
            ...
            exports.cr_dashboard:sendMessageToFaction(1, syntax .. "Un cajero automático se ha vuelto inoperable, ubicación: " .. hexColor .. zoneName)
        end
        ...
        if isElement(element) then 
            ...
            if theX and theY then 
                ...
                if scale > 0 then 
                    ...
                    shadowedText("Out -Of -Service ATM", imageX, imageY, imageX + imageW, imageY + imageH + (35 * scale), tocolor(235, 87, 87, alpha), scale, font, "center", "bottom", math.min(100, alpha))
                    dxDrawText("Out -Of -Service ATM", imageX, imageY, imageX + imageW, imageY + imageH + (35 * scale), tocolor(235, 87, 87, alpha), scale, font, "center", "bottom")
