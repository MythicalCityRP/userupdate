...
function renderLockPicking()
    ...
    shadowedText("Mechones agrietados", barX, barY - 20, barX + barW, barY + barH, tocolor(255, 255, 255, alpha), 1, font2, "left", "top", math.min(100, alpha))
    dxDrawText("Mechones agrietados", barX, barY - 20, barX + barW, barY + barH, tocolor(255, 255, 255, alpha), 1, font2, "left", "top")
    ...
    shadowedText("Mover [A-D] Bloquear peine", panelX + 12, panelY, panelX + panelW, panelY + 50 + panelH, tocolor(255, 255, 255, alpha), 1, font, "left", "bottom", math.min(100, alpha))
    dxDrawText("Mover [A-D] Bloquear peine", panelX + 12, panelY, panelX + panelW, panelY + 50 + panelH, tocolor(255, 255, 255, alpha), 1, font, "left", "bottom")

    shadowedText("[Espacio] Confirmación de la apertura", panelX + 12, panelY, panelX + panelW, panelY + 73 + panelH, tocolor(255, 255, 255, alpha), 1, font, "left", "bottom", math.min(100, alpha))
    dxDrawText("[Espacio] Confirmación de la apertura", panelX + 12, panelY, panelX + panelW, panelY + 73 + panelH, tocolor(255, 255, 255, alpha), 1, font, "left", "bottom")
end
...
function manageLockpickMinigame(state, dbId)
    if state == "init" then 
        ...
        exports.cr_chat:createMessage(localPlayer, "comenzó a romper un cartucho de dinero", 1)
            ...
            if math.abs(lockPickRotation.rotX) == lockPickData.range then 
                if currentHits + 1 < maxHits then 
                    ...
                    if not exports.cr_network:getNetworkStatus() then 
                        ...
                        exports.cr_infobox:addBox("success", "Has abierto con éxito el caja de dinero, obtienes " .. randomMoney .. " dolares.")
                    end
                end
                ...
                exports.cr_infobox:addBox("error", "Durante el descanso, el peine de la cerradura se rompió en la cerradura.")
            end
        end
    end
end
