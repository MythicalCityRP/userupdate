        ...
        if hoverCashBox then 
            if cashBoxes[hoverCashBox] then 
                if lastPickup + 1500 > getTickCount() then 
                    exports.cr_infobox:addBox("error", "No puede obtener los cartuchos de dinero tan rápido.")
                    return
                end
                ...
                if isElement(selectedElement) then 
                    ...
                    exports.cr_chat:createMessage(localPlayer, "Toma un cartucho de dinero del cajero automático.", 1)
                end
                ...
                if (clickedElement:getData("atm >> health") or 100) <= 0 then 
                    ...
                    if table.count(atmCashBoxes) > 0 then 
                        if not hasPermissionToRob("atmRob") then 
                            exports.cr_infobox:addBox("warning", "No tienes permiso a hacerlo.")
                            return
                        end
